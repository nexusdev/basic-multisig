...
contract Person {
    function confirm(BasicMultisig multisig, uint action) {
        multisig.confirm(action);
    }
}

contract BasicMultisigTest is Test, BasicMultisigEvents {
    BasicMultisigFactory factory;
    ...
    function setUp() {
        ...
        dummy = new Dummy();
        eve = new Person();
        factory = new BasicMultisigFactory();
        multisig = factory.createMultisig(members, 2, 24 hours);
        foo(123);
    }
